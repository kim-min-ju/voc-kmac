...
import kr.co.voc.vocapi.common.constants.CommonConstants;
import kr.co.voc.vocapi.coreframework.common.response.ResponseList;
import kr.co.voc.vocapi.coreframework.common.response.ResponseMessage;
...
public class ResponseUtil {
    ...
    public static <T> ResponseObject<T> getResponse(HttpServletRequest request, T data, String messageKey) {
        ...
        if(StringUtils.isEmpty(messageKey)) {
            ...
            return ResponseObject.<T>builder()
                    .api(request.getRequestURI())
                    .errorCode(responseMessage.getErrorCode())
                    .errorMessage(responseMessage.getErrorMessage())
     ...
     * @return
     */
    public static <T> ResponseList<List<T>> getResponseList(HttpServletRequest request, List<T> data) {
        return getResponseList(request, data, null);
    }

    /**
     * 메시지 설정 성공 응답 데이터 목록형
     * @param data
     * @param <T>
     * @return
     */
    public static <T> ResponseList<List<T>> getResponseList(HttpServletRequest request, List<T> data, String messageKey) {
        if(StringUtils.isEmpty(messageKey)) {
            return ResponseList.<List<T>>builder()
                    .api(request.getRequestURI())
                    .data(data)
                    .build();
        }
    ...
    public static <T> ResponseObject<T> getErrorResponse(String apiUrl, BaseDto.Result result, T data) {
        ...
        if(StringUtils.contains(errorCode, "error.")) {
            ...
            responseMessage = ResponseMessage.builder()
                    .errorCode(MessageUtil.getCode("error.9200"))
                    .errorMessage(MessageUtil.getMessage("error.9200"))
...
//                responseMessage.setMessage(errorMessage + "(errorCode : " + errorCode + ")");
    ...
    private static ResponseMessage getResponseMessage(String messageKey, String[] messageArgs) {
        ...
        try {
            ...
            String errorCode = MessageUtil.getSpCode(messageKey);
            String errorMessage = MessageUtil.getMessage("error." + errorCode);
